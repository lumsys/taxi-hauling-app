                        ...
                        <p>Offline Drivers:{{$driver_offlines->count()}}  --> @foreach($driver_offlines as $driver_offline)<span>{{$driver_offline->name}} &nbsp; &nbsp;| &nbsp; &nbsp;</span>@endforeach</p>
                            ...
                            @foreach($triprequests as $triprequest)
                                                    ...
                                                    @if($triprequest->tripRequest == 'pending')
                                                        <td class="text-center smaller"><a class="mr-2 mb-2 btn btn-primary btn-sm" href="{{route('trip.approvetriprequest',$triprequest->id)}}">Approve</a></td>
                                                    @endif
                                                    <td class="text-center smaller"><a class=" mb-2 btn btn-danger btn-sm" href="{{route('trip.declinetriprequest',$triprequest->id)}}" onclick="return confirm('Are you sure you want to delete this?');">Decline</a></td>
